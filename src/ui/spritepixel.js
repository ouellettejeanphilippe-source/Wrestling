...
function cutArms(grid, kind, from, to) {
  for (let y = from; y <= to && y < ART_H; y++) {
    ...
    // Le contour du torse partait avec les bras : il vivait sur la colonne
    // extérieure du bras, pas sur celle du buste. Sans lui le tronc n'a plus
    // de silhouette — un t-shirt sans bord, qui bave sur le fond. On le
    // repose ici ; une posture qui ramène un bras devant repeindra par-dessus.
    grid[y][span[0] - 1] = 'K';
    grid[y][span[1] + 1] = 'K';
  }
}
